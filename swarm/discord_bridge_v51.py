...
class SwarmDiscordBridge(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Verifica se é resposta às perguntas do Ralph (AUTONOMY FIX)
        if await self._handle_question_answer(message):
            pending_queue.mark_processed(str(message.id))
            return
    ...
    async def _handle_question_answer(self, message: discord.Message):
        """
        Detecta quando usuário responde às perguntas do Ralph.
        Cria o plano de execução automaticamente.
        """
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            
            # Busca tasks aguardando perguntas e verifica o metadata
            discord_channel_id = str(message.channel.id)
            logger.info(f"🔍 Verificando respostas no canal {discord_channel_id}")
            
            cursor.execute("""
                SELECT id, task_code, original_request, questions_asked, metadata
                FROM swarm_tasks
                WHERE status = 'awaiting_questions'
                ORDER BY created_at DESC
                LIMIT 5
            """)
            
            rows = cursor.fetchall()
            logger.info(f"🔍 Encontradas {len(rows)} tasks aguardando_questions")
            
            matching_task = None
            
            for row in rows:
                task_id, task_code, original_request, questions_asked, metadata_json = row
                try:
                    metadata = json.loads(metadata_json or '{}')
                    task_channel_id = metadata.get('discord_channel_id')
                    logger.info(f"🔍 Task {task_code}: canal {task_channel_id} vs mensagem {discord_channel_id}")
                    # Verifica se a task foi criada neste canal do Discord
                    if str(task_channel_id) == discord_channel_id:
                        matching_task = row
                        logger.info(f"✅ Match encontrado: {task_code}")
                        break
                except Exception as e:
                    logger.warning(f"⚠️ Erro ao parse metadata da task: {e}")
                    continue
            
            if not matching_task:
                logger.info(f"❌ Nenhuma task encontrada para o canal {discord_channel_id}")
                conn.close()
                return False
            
            task_id, task_code, original_request, questions_asked, metadata_json = matching_task
            
            logger.info(f"📝 Detectada resposta às perguntas da task {task_code}")
            ...
            await message.channel.send(
                f"📝 **Respostas recebidas!** Criando plano de execução para `{task_code}`..."
            )
            
            conn.close()
            
            # Cria o plano imediatamente (não espera "aprovado")
            await self._create_execution_plan(task_id, original_request, answers)
    ...
    async def _create_execution_plan(self, task_id: int, original_request: str, user_answers: str):
        """Cria plano de execução após receber respostas"""
        try:
            from coordination_engine import SwarmCoordinator
            
            coordinator = SwarmCoordinator()
            
            # Analisa a tarefa com as respostas do usuário
            full_context = f"""Tarefa: {original_request}

Respostas do usuário:
{user_answers}"""
            
            plan = coordinator.analyze_task(full_context)
            
            # Gera o plano em formato texto
            plan_text = f"""📋 **Plano de Execução**

**Complexidade:** {plan.complexity.value}
**Agents necessários:** {', '.join(plan.agents_required)}
**Estratégia:** {plan.execution_strategy}
**Passos estimados:** {plan.estimated_steps}

Para aprovar, digite: **aprovado**
Para rejeitar, explique o que precisa mudar."""
            
            # Envia no Discord
            await self._send_plan_to_discord(task_id, plan_text)
            ...
            logger.info(f"✅ Plano criado para task {task_id}")
            
        except Exception as e:
            logger.error(f"Erro ao criar plano: {e}")
    
    async def _send_plan_to_discord(self, task_id: int, plan_text: str):
        """Envia plano para o canal do Discord"""
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("""
                SELECT task_code, metadata
                FROM swarm_tasks
                WHERE id = ?
            """, (task_id,))
            row = cursor.fetchone()
            ...
            task_code, metadata_json = row
            
            # Extrai discord_channel_id do metadata
            metadata = json.loads(metadata_json or '{}')
            discord_channel_id = metadata.get('discord_channel_id')
            ...
            if discord_channel_id:
                ...
                if channel:
                    embed = discord.Embed(
                        title=f"📋 Plano - {task_code}",
                        description=plan_text[:4000],
                        color=0x3498db
                    )
                    await channel.send(embed=embed)
                    logger.info(f"✅ Plano enviado para Discord: {task_code}")
                else:
                    logger.warning(f"⚠️ Canal não encontrado: {discord_channel_id}")
            else:
                logger.warning(f"⚠️ Sem discord_channel_id no metadata da task {task_code}")
                    
        except Exception as e:
            logger.error(f"Erro ao enviar plano: {e}")
